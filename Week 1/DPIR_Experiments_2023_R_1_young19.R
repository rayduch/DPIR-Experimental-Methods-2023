...
# The value of scipen is the number of orders of ten smaller
# (i.e. decimal places preceded by 0's) required to switch to scientific notation.
...
## which includes weights as 1/p and heteroskedastic robust SEs
## (treatment groups are of unequal size)
...
## (using actual distribution of the outcome in the data to compute a test statistic) 
...
ates <- tibble(
  ...
  ate = c(round(dim_hg, 3), round(m_hg$coefficients[2], 3), round(mod1g$coefficients[2], 3), -0.0545,
          ...
          ## coef() extracts coefficient from model objects
...
# visualises the distribution of data over a continuous interval
...
p <- melt(dat[,c('surveyor_id', 'treat_assign', acts)],
          id=c('surveyor_id', 'treat_assign'))
